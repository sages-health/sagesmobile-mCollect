...
import org.odk.collect.android.tasks.InstanceSMSerTask;
...
public class InstanceSMSerActivity extends Activity implements InstanceSMSerListener {
    ...
    private InstanceSMSerTask mInstanceSMSerTask;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mInstanceSMSerTask = (InstanceSMSerTask) getLastNonConfigurationInstance();
        if (mInstanceSMSerTask == null) {
            // setup dialog and upload task
            showDialog(PROGRESS_DIALOG);
            mInstanceSMSerTask = new InstanceSMSerTask();

            Long[] toSendArray = new Long[mInstancesToSend.size()];
            mInstancesToSend.toArray(toSendArray);
            mInstanceSMSerTask.execute(toSendArray);
        } 
    }
                ...
                DialogInterface.OnClickListener loadingButtonListener =
                    new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            dialog.dismiss();
                            mInstanceSMSerTask.cancel(true);
                            mInstanceSMSerTask.setSMSerListener(null);
                            finish();
                        }
                    };
    ...
    protected void onResume() {
        if (mInstanceSMSerTask != null) {
            mInstanceSMSerTask.setSMSerListener(this);
        }
        if (mAlertShowing) {
